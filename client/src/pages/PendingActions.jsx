...
export default function PendingActions(){
  ...
  async function load(){
    ...
    try{
      ...
      const { data: reqData } = await api.post('/members/requests/filter', { role })
    ...
    <div className="space-y-4">
      ...
      {role === 'accountant' && (
        <div className="card">
          ...
          {!loading && !donations.length && <div className="text-sm text-slate-500">No pending donations found.</div>}
          <div className="space-y-2">
            <div className="flex items-center gap-2 mb-2">
              <input type="checkbox" onChange={(e)=>{ if (e.target.checked) setSelectedDonations(new Set(donations.map(x=>x._id))) ; else setSelectedDonations(new Set()) }} />
            ...
            {donations.map(d=> (
              <div key={d._id} className="card flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-slate-500">Added By: {d.addedByName || d.addedBy || 'N/A'}</div>
                </div>
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-2">
            <input type="checkbox" onChange={(e)=>{ if (e.target.checked) setSelectedRequests(new Set(requests.map(x=>x._id))) ; else setSelectedRequests(new Set()) }} />
            <button className="btn" onClick={bulkApproveRequests}>{role === 'accountant' ? 'Bulk Confirm Payments' : 'Bulk Approve'}</button>
          </div>
          {requests.map(r=> (
            <div key={r._id} className="card flex items-center justify-between">
              <div>
                <div className="font-medium">{r.name} • ₹{r.membershipFee}</div>
                <div className="text-sm text-slate-500">Requested By: {r.createdByName || r.createdBy}</div>
                <div className="text-sm text-slate-500">Status: {r.status}</div>
              </div>
              <div className="flex gap-2 items-center">
                <input type="checkbox" checked={selectedRequests.has(r._id)} onChange={()=>toggleRequest(r._id)} />
                {(role === 'president' || role === 'secretary') && (
                  ...
                  >
                    {processingRequests.has(r._id) ? 'Processing...' : 'Approve'}
                  </button>
                )}
                {role === 'accountant' && (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setProcessingRequests(s);
                      try {
                        await api.post(`/members/requests/${r._id}/confirm-payment`);
                        setRequests(prev => prev.filter(x => x._id !== r._id));
                      } catch (e) {
                        alert(e.response?.data?.message || e.message);
                      } finally {
                  ...
                  >
                    {processingRequests.has(r._id) ? 'Processing...' : 'Confirm Payment'}
                  </button>
                )}
              </div>
            </div>

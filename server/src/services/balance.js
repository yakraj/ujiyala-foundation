...
export async function computeSummary() {
  ...
  const partialExpenses = await Expense.find({ status: "partial" });
  const partialCount = partialExpenses.length;
  const partialRemaining = partialExpenses.reduce(
    (sum, e) => sum + ((e.totalAmount || e.amount) - e.amount),
    0
  );
